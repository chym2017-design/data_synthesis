...
用法:
    python test_llm.py                                    # 测试所有模型（默认 prompt）
    python test_llm.py --prompt "你好"                      # 自定义 prompt
    python test_llm.py --model qwen-plus                    # 指定单个模型
    python test_llm.py --stream                            # 流式调用
    python test_llm.py --list                              # 列出可用模型
    python test_llm.py --prompt-file prompt.txt --model glm-5  # 从文件读 prompt
"""
...
CONFIG_PATH = SCRIPT_DIR / "configs" / "llm_config.yaml"
